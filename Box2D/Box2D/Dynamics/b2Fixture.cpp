	...
	case b2Shape::e_edge:
		{
			...
			b2Log("    shape.m_radius = %.15lef;\n", s->m_radius);
			b2Log("    shape.m_vertex0.Set(%.15lef, %.15lef);\n", s->m_vertex0.vector[0], s->m_vertex0.vector[1]);
			b2Log("    shape.m_vertex1.Set(%.15lef, %.15lef);\n", s->m_vertex1.vector[0], s->m_vertex1.vector[1]);
			b2Log("    shape.m_vertex2.Set(%.15lef, %.15lef);\n", s->m_vertex2.vector[0], s->m_vertex2.vector[1]);
			b2Log("    shape.m_vertex3.Set(%.15lef, %.15lef);\n", s->m_vertex3.vector[0], s->m_vertex3.vector[1]);
	...
	case b2Shape::e_polygon:
		{
			...
			{
				b2Log("    vs[%d].Set(%.15lef, %.15lef);\n", i, s->m_vertices[i].vector[0], s->m_vertices[i].vector[1]);
			}
	...
	case b2Shape::e_chain:
		{
			...
			b2Log("    shape.m_prevVertex.Set(%.15lef, %.15lef);\n", s->m_prevVertex.vector[0], s->m_prevVertex.vector[1]);
			b2Log("    shape.m_nextVertex.Set(%.15lef, %.15lef);\n", s->m_nextVertex.vector[0], s->m_nextVertex.vector[1]);
